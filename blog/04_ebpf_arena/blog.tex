...
programs to allocate memory pages; similar functionality as \myC{mmap} and
\myC{munmap}~\cite{arenapatch}.
...
cause the generation of ``\myC{bpf_arena_cast_user/kern}'' instrucitons. The
``\myC{__arena}'' used in the examples (and the acompanying repository),
...
be attached to a hook and instead is invoked using the \myC{BPF_PROG_TEST_RUN}
system call~\cite{ebpf_docs_bpf_prog_run}.

Listing~\ref{lst:mogu} demonstrates an eBPF program using Arena. In this
program, a global variable (``\myC{flag_initialized}'') tracks if it is the first
time the program is invoked or not. At the first invocation a page is allocated
and its address is kept in ``\myC{mem}'' global variable. The allocated memory
...
which the program is running in a \myC{NAPI}. But, this limitation does not mean
...
\myC{bpf_arena_alloc_pages} helper for allocating the memory. Instead, it is
...
\begin{enumerate}
    ...
    \item The second program is using a helper (\myC{my_kfunc_reg_arena}) that is
...
If we remove the the call to the \myC{my_kfunc_reg_arena} in
...
helpers. Other helper functions such as \myC{bpf_map_lookup_elem} also cause
...
memory (the address we found using \myC{bpf_map__initial_value}) at offset 0
and 4096 to allocate the two pages. Accessing the memory outside of the Arena
region will cause a \myC{SIGSEGV} (Address boundary error).
